...
<template>
  <div class="setting_base full-height">
    <q-splitter v-model="splitterModel" :limits="[20, 30]" class="settings-splitter" separator-class="bg-grey-4">
      <template v-slot:before>
        <q-tabs v-model="tab" vertical class="text-teal settings-tabs" active-color="primary" indicator-color="primary">
          <q-tab v-for="item in tabItems" :key="item.name" :name="item.name" :icon="item.icon"
            :label="$t(`settingPage.${item.label}`)" :class="{ 'active-tab': tab === item.name }" />
        </q-tabs>
      </template>
        ...
        <q-tab-panels v-model="tab" animated swipeable vertical transition-prev="jump-up" transition-next="jump-up"
          ...
          <q-tab-panel name="language" class="q-pa-md">
            <div class="panel-content">
              ...
              <q-select color="purple-12" v-model="model" filled :options="options" :label="$t('settingPage.name')"
                emit-value map-options class="language-select">
          ...
          <q-tab-panel name="theme" class="q-pa-md">
            <div class="panel-content">
              ...
              <q-btn-toggle v-model="currentTheme" :options="[
                { label: $t('settingPage.theme.light'), value: 'light' },
                { label: $t('settingPage.theme.dark'), value: 'dark' }
              ]" spread color="primary" @update:model-value="changeTheme" class="theme-toggle" />
            </div>
          </q-tab-panel>
            ...
            <div class="panel-content">
              ...
              <div class="row q-col-gutter-md">
                <div class="col-12">
                  ...
                  <q-slider v-model="fontSize" :min="12" :max="20" :step="1" label label-always color="primary"
                    @update:model-value="changeFontSize" class="font-slider" />
                </div>
              ...
              <q-card flat bordered class="proxy-settings">
                <q-card-section>
                  <div class="row q-col-gutter-md">
                    <!-- 启用代理开关 -->
                    <div class="col-12">
                      <q-toggle v-model="proxyEnabled" :label="$t('settingPage.proxy.enable')" color="primary" />
                    </div>

                    <!-- 代理协议 -->
                    <div class="col-12 col-sm-4">
                      <q-select v-model="proxyProtocol" :options="['http', 'https']"
                        :label="$t('settingPage.proxy.protocol')" :disable="!proxyEnabled" outlined dense
                        class="q-mb-md" />
                    </div>

                    <!-- 代理主机和端口 -->
                    <div class="col-12 col-sm-4">
                      <q-input v-model="proxyHost" :label="$t('settingPage.proxy.host')"
                        :placeholder="$t('settingPage.proxy.placeholder.host')" :disable="!proxyEnabled" outlined
                        dense />
                    </div>
                    <div class="col-12 col-sm-4">
                      <q-input v-model="proxyPort" type="number" :label="$t('settingPage.proxy.port')"
                        :placeholder="$t('settingPage.proxy.placeholder.port')" :disable="!proxyEnabled" outlined
                        dense />
                    </div>

                    <!-- 认证信息 -->
                    <div class="col-12">
                      <q-expansion-item :label="$t('settingPage.proxy.auth')" :disable="!proxyEnabled" dense>
                        <q-card>
                          <q-card-section>
                            <div class="row q-col-gutter-md">
                              <div class="col-12 col-sm-6">
                                <q-input v-model="proxyUsername" :label="$t('settingPage.proxy.username')"
                                  :placeholder="$t('settingPage.proxy.placeholder.username')" outlined dense />
                              </div>
                              <div class="col-12 col-sm-6">
                                <q-input v-model="proxyPassword" type="password"
                                  :label="$t('settingPage.proxy.password')"
                                  :placeholder="$t('settingPage.proxy.placeholder.password')" outlined dense />
                              </div>
                            </div>
                          </q-card-section>
                        </q-card>
                    ...
                    <div class="col-12">
                      <div class="row justify-end q-gutter-sm">
                        <q-btn :label="$t('settingPage.proxy.testConnection')" color="secondary"
                          :disable="!proxyEnabled || !isValidProxy" @click="testProxyConnection" flat />
                        <q-btn :label="$t('settingPage.proxy.save')" color="primary" :disable="!isValidProxy"
